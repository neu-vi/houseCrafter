    ...
    def __getitem__(self, index):
        ...
        if self.fix_sample:
            if self.T_out > 1:
                ...
                index_inputs = indexes[
                    self.T_out - 1 : self.T_in + self.T_out - 1
                ]  # one overlap identity
        else:
            assert self.T_in + self.T_out <= total_view
            # training with replace, including identity
            indexes = np.random.choice(
                range(total_view), self.T_in + self.T_out, replace=True
            )
        ...
        try:
            ...
            for i, index_input in enumerate(index_inputs):
                input_im = self.process_im(
                    self.load_im(
                        os.path.join(filename, "%03d.png" % index_input), color
                    )
                )
            ...
            for i, index_target in enumerate(index_targets):
                ...
                target_Ts.append(
                    self.get_pose(
                        np.concatenate(
                            [target_RT[:3, :], np.array([[0, 0, 0, 1]])], axis=0
                        )
                    )
                )
        except:
            ...
            filename = os.path.join(
                self.root_dir, "0a01f314e2864711aa7e33bace4bd8c8"
