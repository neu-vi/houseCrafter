...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        """
        in_pos3d: b (t_in h w) d,  d is attn dim
        ...
        if posemb is not None:
            ...
            # NOTE: in this implementation of eschernet the dot product between q and k is
            # q @ (p_out)^(-T) p_in^T @ k.T = q @ (c_q2c_k)^T @ k.T
            # here p_out is w2c_q and p_in is w2c_k
            # so it can be interpreted as a dot product between key and query in the c_k coordinate
            # so in_pos3d should be in the c_k coordinate and added before applying cape_embed to the key
            # out_pos3d should be in c_q coordinate and added before applying cape_embed to the query
            if in_pos3d is not None and not self_attn:
                key = key + in_pos3d
