...
from dataset import RCNScannetData, ScannetData
...
@torch.no_grad()
def log_validation(validation_dataloader, vae, image_encoder, feature_extractor, unet, args, accelerator, weight_dtype, split="val", use_ray=False):
    ...
    for T_in_val in [1, args.T_in_val//2, args.T_in_val]:   # eval different number of given views
        for valid_step, batch in tqdm(enumerate(validation_dataloader)):
            ...
            kwargs = {}
            if use_ray:
                target_rays = [v.to(dtype=weight_dtype) for k,v in batch.items() if "target_ray" in k]
            ...
            for _ in range(args.num_validation_images):
                with torch.autocast("cuda"):
                    image = pipeline(input_imgs=input_image, prompt_imgs=input_image, poses=[[pose_out, pose_out_inv], [pose_in, pose_in_inv]], height=h, width=w, T_in=T_in, T_out=pose_out.shape[1],
                                     guidance_scale=args.guidance_scale, num_inference_steps=50, generator=generator, output_type="numpy", **kwargs).images
...
def main(args):
    ...
    kwargs = {}
    if args.use_ray:
        kwargs["down_block_types"]=['RCNCrossAttnDownBlock2D', 'RCNCrossAttnDownBlock2D', 'RCNCrossAttnDownBlock2D', 'RCNDownBlock2D']
        kwargs["up_block_types"]=['RCNUpBlock2D', 'RCNCrossAttnUpBlock2D', 'RCNCrossAttnUpBlock2D', 'RCNCrossAttnUpBlock2D']
        kwargs["mid_block_type"]="RCNUNetMidBlock2DCrossAttn"
        kwargs["m_bands"]=args.m_bands
        kwargs["d_bands"]=args.d_bands
        kwargs["use_rcn"]=True
    ...
    #todo: change from objaverse to scannet
    if args.use_ray:
        train_dataset = RCNScannetData(scene_num_views=args.scene_num_views, root_dir=args.train_data_dir, image_transforms=image_transforms, T_in=T_in, T_out=T_out)
        train_log_dataset = RCNScannetData(scene_num_views=args.scene_num_views, root_dir=args.train_data_dir, image_transforms=image_transforms, mode="val", T_in=T_in_val, T_out=T_out)
        validation_dataset = RCNScannetData(scene_num_views=args.scene_num_views, root_dir=args.val_data_dir, image_transforms=image_transforms, mode="val", T_in=T_in_val, T_out=T_out)
    else:
        train_dataset = ScannetData(scene_num_views=args.scene_num_views, root_dir=args.train_data_dir, image_transforms=image_transforms, T_in=T_in, T_out=T_out)
        train_log_dataset = ScannetData(scene_num_views=args.scene_num_views, root_dir=args.train_data_dir, image_transforms=image_transforms, mode="val", T_in=T_in_val, T_out=T_out)
        validation_dataset = ScannetData(scene_num_views=args.scene_num_views, root_dir=args.val_data_dir, image_transforms=image_transforms, mode="val", T_in=T_in_val, T_out=T_out)
    # for training
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, image_encoder):
                ...
                if args.use_ray:
                    target_rays = [v.to(dtype=weight_dtype) for k,v in batch.items() if "target_ray" in k]
                    target_rays = [einops.rearrange(x, 'b t c h w -> (b t) c h w', t=T_out) for x in target_rays]
                    kwargs["target_rays"] = target_rays
