"""
modified the noise sampling and decode in the pipeline to output the depth
"""
...
class Zero1to3StableDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_image(self, image, depth_mask_cond, device, num_images_per_prompt, do_classifier_free_guidance, cond_rays=None):
        ...
        image_embeddings = self.image_encoder(image, rays=cond_rays, depth_mask = depth_mask_cond)#.last_hidden_state  # bt, 768, 12, 12
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None, gaussian_clip=0.0):
        """
        gaussian_clip: float, clip the gaussian noise to [-gaussian_clip, gaussian_clip] if gaussian_clip > 0
        """
        ...
        if latents is None:
            ...
            if gaussian_clip > 0:
                latents = torch.clamp(latents, -gaussian_clip, gaussian_clip)
        else:
    ...
    def prepare_ddim_inversion_latents(
        self, 
        warp_img, 
        poses,
        device,
        ...
        args:
            wrap_img: torch.Tensor, (b, t, c, h, w)
            wrap_depth: torch.Tensor, (b, t, h, w)
        """
        assert self.inverse_scheduler is not None, "Inverse scheduler is not defined."
        # encode warp_img and warp_depth
        warp_img = einops.rearrange(warp_img, 'b t c h w -> (b t) c h w')
        latents = self.vae.encode(warp_img).latent_dist.mode()
        latents = latents * self.vae.config.scaling_factor
        if warp_depth is not None:
            warp_depth = einops.repeat(warp_depth, 'b t h w -> (b t) c h w', c=3)
            depth_latent = self.vae.encode(warp_depth).latent_dist.mode()
            depth_latent = depth_latent * self.vae.config.scaling_factor
            latents = torch.cat([latents, depth_latent], dim=1)
        
        # run ddim inversion
        self.inverse_scheduler.set_timesteps(num_inference_steps, device=device)
        timesteps = self.inverse_scheduler.timesteps
        
        ## prepare args for ddim inversion
        extra_step_kwargs = {}
        accepts_eta = "eta" in set(inspect.signature(self.inverse_scheduler.step).parameters.keys())
        if accepts_eta:
            extra_step_kwargs["eta"] = eta
        accepts_generator = "generator" in set(inspect.signature(self.inverse_scheduler.step).parameters.keys())
        if accepts_generator:
            extra_step_kwargs["generator"] = generator
    ...
    def __call__(
        ...
        Args:
            ...
            target_rays (`list`, *optional*): 
                list of 4 tensors, each of shape [B,T,6,Hi,Wi]
            cond_rays (`list`, *optional*): 
                list of 4 tensors, each of shape [B*T,6,Hi,Wi]
            layouts: (`dict`, *optional*)
                dict of {
                    "layout_cls": tensor of shape B,T,n,H,W,
                    "layout_pos": tensor of shape B,T,n,c,H,W,
                }
            wrap_img: torch.Tensor, (b, t, c, h, w)
            wrap_depth: torch.Tensor, (b, t, h, w)
        ...
        if isinstance(input_imgs, PIL.Image.Image):
            ...
            batch_size = len(input_imgs) // T_in
        else:
            batch_size = input_imgs.shape[0] // T_in
        ...
        # print('use depth_mask_cond', depth_mask_cond is not None)
        prompt_embeds = self._encode_image(prompt_imgs, depth_mask_cond, device, num_images_per_prompt, do_classifier_free_guidance, cond_rays=cond_rays)
        prompt_embeds = einops.rearrange(prompt_embeds, '(b t) l c -> b (t l) c', t=T_in)
        if rgbd_cross_attention:
            prompt_embeds = einops.repeat(prompt_embeds, 'b l c -> (b k) l c', k=2)
        if ignore_prompts:
        ...
        if in_pos3d is not None:
            ...
            if ignore_prompts:
                in_pos3d = torch.zeros_like(in_pos3d)
        ...
        if layouts is not None:
                ...
                for key in keys:
                    _layout =  layouts[key]
                    _layout = einops.rearrange(_layout, "b t ... -> (b t) ...")
                    in_layouts[key] = _layout
        if target_rays is not None:
            target_rays = [einops.rearrange(ray, "b t c h w -> (b t) c h w") for ray in target_rays]

        # type class 0 for rgb and 1 for depth
        type_classes = torch.zeros(batch_size, 2, T_out, device=device, dtype=torch.long)
        type_classes[:, 1] = 1
        type_classes = einops.rearrange(type_classes, "b k t -> (b k t)")
        if do_classifier_free_guidance:
            ...
            if target_rays is not None:
                target_rays = [einops.repeat(ray, "b c h w -> (2 b) c h w") for ray in target_rays]
            if in_layouts is not None:
                keys = list(in_layouts)
                for key in keys:
                    _layout =  in_layouts[key]
                    _layout = torch.cat([torch.zeros_like(_layout), _layout])
                    in_layouts[key] = _layout
            if in_pos3d is not None:
                in_pos3d = torch.cat([torch.zeros_like(in_pos3d), in_pos3d])
            type_classes = einops.repeat(type_classes, "b -> (2 b)")
        if not rgbd_cross_attention:
            type_classes = None
        # 4. Prepare timesteps
        ...
        if ddim_inversion and (not ignore_prompts):
            raise NotImplementedError("DDIM inversion is not supported.")
            assert latents is None, "latents should not be provided when ddim_inversion is True"
            ## make nul image condition and poses
            if do_classifier_free_guidance:
                null_layout = {k: torch.zeros_like(v.chunk(2)[0]) for k, v in in_layouts.items()} if in_layouts is not None else None
                null_prompt_embeds = torch.zeros_like(prompt_embeds.chunk(2)[0])
                [_pose_out, _pose_out_inv], [_pose_in, _pose_in_inv] = poses
                _poses = [
                    [_pose_out.chunk(2)[0], _pose_out_inv.chunk(2)[0]], 
                    [_pose_in.chunk(2)[0], _pose_in_inv.chunk(2)[0]]
                ]
                _target_rays = [ray.chunk(2)[0] for ray in target_rays]
                _in_pos3d = in_pos3d.chunk(2)[0] if in_pos3d is not None else None
            else:
                null_layout = {k: torch.zeros_like(v) for k, v in in_layouts.items()} if in_layouts is not None else None
                null_prompt_embeds = torch.zeros_like(prompt_embeds)
                _poses = poses
                _target_rays = target_rays
                _in_pos3d = in_pos3d
            latents = self.prepare_ddim_inversion_latents(
                warp_img=warp_img,
                ...
                in_pos3d=_in_pos3d,
                target_rays=_target_rays,
                eta=eta,
                generator=generator,
            )

        latents = self.prepare_latents(
            batch_size * T_out, # todo use t_out
            4 if latents is None else latents.size(1),
            height,
            width,
            prompt_embeds.dtype,
            device,
        ...
        if rgbd_cross_attention:
            assert output_depth and latents.size(1) == 4
            latents = einops.repeat(latents, "(b t) c h w -> (b k t) c h w", k=2, t=T_out)
        elif output_depth and latents.size(1) == 4:
            latents = einops.repeat(latents, "b c h w -> b (n c) h w", n=2)
        ...
        if output_type == "latent":
            ...
            # 8. Post-processing
            if output_depth:
                raise ValueError("output_depth is not supported for output_type='pil', use 'latent' or 'numpy' instead.")
            ...
            # 8. Post-processing
            if output_depth:
                # the depth interleave with image along the batch dimension
                if not rgbd_cross_attention:
                    latents = einops.rearrange(latents, "b (n c) h w -> (b n) c h w", n=2)
            image = self.decode_latents(latents)
