...
def render_scene_from_pose(scene_id, args, seed, batch_size=12):
    ...
    with env.begin() as txn:
        ...
        key_counters = Counter(exist_keys)
        # TODO: change after updating the key
        exist_keys = [key for key, count in key_counters.items() if count == 2]
...
def save_data_per_location(db, data, poses, loc_key):
    ...
    for i, (pose, depth) in enumerate(zip(poses, data["depth"])):
...
def save_data_per_pose_batch(db, data, poses, keys):
    ...
    out_txn = db.begin(write=True)
    for key, pose, depth in zip(keys, poses, data["depth"]):
...
if __name__ == "__main__":
    args = parse_args()
    assert (
        args.locations_graph_folder or args.poses_folder
    ), "Need locations or poses folder"
    assert not (
        args.locations_graph_folder and args.poses_folder
    ...
    # if True:
    try:
        if args.locations_graph_folder:
            #     print("run location")
