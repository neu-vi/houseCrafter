...
class Room:
    ...
    def points_in_room(self, points: np.ndarray) -> np.ndarray:
        """
        check if 2d points in xz plane are in the room

        args:
            points: np.array (n,2) x,z coords of points

        return np.array (n,) bool
        """
    ...
    def get_locations_on_floor(
        ...
        inliers = self.points_in_room(xz)
